/* =====================================================================
   ...
   servicios estaban vivos; solo de descubrirlo chocando.
   ...
   comeria el limite de subrequests, y ademas asi se ve el avance en vivo en
   lugar de esperar a que terminen todas.
...
export async function renderApiStatus(user) {
  ...
  pintar();
  revisarTodas();          // arranca solo: nadie entra acá para no revisar
}
...
function pintar() {
  ...
  const hechos = APIS.filter(a => ['ok', 'fail', 'cfg'].includes(RES[a.code].estado)).length;
  const fallas = APIS.filter(a => ['fail', 'cfg'].includes(RES[a.code].estado)).length;
  ...
  const resumen = !CORRIENDO && hechos === APIS.length
    ? (fallas
      ? `<span class="as-pill bad">${fallas} con problema${fallas === 1 ? '' : 's'}</span>`
      : `<span class="as-pill good">Todo responde</span>`)
    : `<span class="as-pill run">Revisando… ${hechos}/${APIS.length}</span>`;
  ...
  body.innerHTML = `
    <div class="as-head">
      ...
      <button class="as-btn" id="asAgain" ${CORRIENDO ? 'disabled' : ''}>↻ Revisar de nuevo</button>
    </div>
  ...
  document.getElementById('asAgain')?.addEventListener('click', () => {
    if (CORRIENDO) return;
    APIS.forEach(a => { RES[a.code] = { estado: 'wait' }; });
    pintar();
    revisarTodas();
...
async function revisarTodas() {
  CORRIENDO = true;
  for (const a of APIS) {
    RES[a.code] = { estado: 'run' };
    pintar();
    const r = await api({ action: 'check', code: a.code });
    if (!r || !r.ok) {
      RES[a.code] = { estado: 'fail', reason: (r && r.error) || 'No se pudo consultar.' };
    } else if (r.config) {
      RES[a.code] = { estado: 'cfg', reason: r.reason };
    } else if (r.ok_api) {
      RES[a.code] = { estado: 'ok', ms: r.ms, rows: r.rows, reason: r.reason || null };
    } else {
      RES[a.code] = { estado: 'fail', ms: r.ms, reason: r.reason || 'No respondió.' };
    }
    pintar();
  }
  CORRIENDO = false;
